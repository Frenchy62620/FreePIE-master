...
namespace FreePIE.Core.Plugins.ScriptAuto
{
    public class ScriptKeyboard
    {
        ...
        {
            bool previouslyPressed = dico.TryGetValue((int)code, out bool isdown) && isdown;
            if (!isdown)
                dico[code] = (isdown = false);
            else
                isdown = isKeyDown((SharpDX.DirectInput.Key)code);

            return !previouslyPressed && isdown;
        }
        ...
        public void Keyboard()
        {
            ...
            {
                if (IsPressed((int)SharpDX.DirectInput.Key.NumberPadEnter))
                {
                    NextAction();
                    return;
                }
                ...
                if (((result[0] >= (int)Key.A && result[0] <= (int)Key.Z) || result[0] == (int)Key.Semicolon) && IsPressed(scan[result[0]]))
                {
                    string s;
