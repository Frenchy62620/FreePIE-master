...
namespace FreePIE.GUI.Common.Visiblox
{
    ...
    public class MovePointBehaviour : BehaviourBase
    {
        ...
        {
            if ((int)Chart.Tag != 4) return; // Move = 1, Del = 2 and Add = 4
        ...
        {
            if ((int)Chart.Tag > 2) return; // Move = 1, Del = 2 and Add = 4
        ...
        public override void MouseMove(Point position)
        {
            ...
            if (!_leftMouseDown || _selectedPoint == null || (int)Chart.Tag > 1)
            {
                ...
                var args0 = new PointSelectedEventArgs { Point = new Core.Model.Point((double)p.X, (double)p.Y), Selected = false };
