...
using FreePIE.Plugin_DCSPlugin.ScriptAuto;
using FreePIE.Plugin_DCSPlugin.DCSBIOS;

namespace FreePIE.Plugin_DCSPlugin
{
 ...
 //           Function();
    ...
    public class DCSGlobal
    {
        ...
        {
            plugin.SendDCSCommand(command + " " + (1 - getData(address, mask, shift)));
        }
        ...
        {
            string result = plugin.GetData(address, length);
            return result;
        }

        public int getData(uint address, int mask, int shift)//string identifier, int output = 0)
        {
            int result = plugin.GetData(address, mask, shift);
            return result;
        }
        ...
        public bool IsActionFinished() => cmd == '\0';
        ...
        {
            plugin.InitSerial(baudrate, port);
        }
