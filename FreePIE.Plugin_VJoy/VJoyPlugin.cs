...
using FreePIE.Plugin_VJoy.ScriptAuto;
using vJoyInterfaceWrap;
using static FreePIE.CommonTools.GlobalTools;
using FreePIE.Plugin_VJoy.Globals;
using FreePIE.CommonStrategy;

namespace FreePIE.Plugin_VJoy
{
    [GlobalEnum]
    public enum Pov
    {
    ...
    public sealed class VJoyGlobalHolder : IDisposable
    {
        ...
        public void Dispose() => _joystick.RelinquishVJD(Index);

        public void KeepDown(bool trigger, int button, int duration)
        {
            if (trigger)
                PressAndRelease(button, duration);
        }
    }
    ...
    public class VJoyGlobal
    {
                               ...
                               int? slider = null, int? dial = null)
        {
            ...
            this.ry = ry ?? this.ry;
            this.rz = rz ?? this.rz;
            this.sl0 = slider ?? this.sl0;
            this.sl1 = dial ?? this.sl1;
        }
        ...
        // ************************ setAnalogPov ***********************************************
        public void setAnalogPov(int numpov, int value) => holder.SetContinuousPov(numpov, value);

        // Update data in mode DIFFERED **********************************************************
        public void _updateVJD() => holder._UpdateVJD();

        // DIVERS **********************
        public void setPressed(int button, int delay = -1) => holder.PressAndRelease(button, delay);
        public void keepDown<T>(bool trigger, int button, int duration) => holder.KeepDown(trigger, button, duration);
